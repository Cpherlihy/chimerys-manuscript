...
pathToPdResult_pcms <- file.path(dataPath, "LFQ_Bench_human/Chimerys/LFQ_01_CHIMERYS_localPcmFdr.pdResult")
...
psms_t <- merge(psms_t,
                ...
                by = 'PeptideID',
                all.x = T)
uniqueN(psms_t[qValue <= 0.01, TargetPeptideGroupsPeptideGroupID]) # 71612
uniqueN(modPeps_t[qValue <= 0.01, PeptideGroupID]) # 66618

round((uniqueN(psms_t[qValue <= 0.01, TargetPeptideGroupsPeptideGroupID]) /
         uniqueN(modPeps_t[qValue <= 0.01, PeptideGroupID]) * 100), 2) # 107.5

uniqueN(psms_t[qValue <= 0.01, TargetPeptideGroupsPeptideGroupID]) -
  uniqueN(modPeps_t[qValue <= 0.01, PeptideGroupID]) # 4994
...
uniqueN(modPeps_t[qValue <= 0.01, PeptideGroupID]) # 66618
uniqueN(modPeps_t[qValue <= 0.01, MODPEP_J_ID]) # 66618
uniqueN(pcms_t[qValue <= 0.01, MODPEP_J_ID]) # 67239

round((uniqueN(pcms_t[qValue <= 0.01, MODPEP_J_ID])/
         uniqueN(modPeps_t[qValue <= 0.01, PeptideGroupID]) * 100), 2) # 100.93
...
# 83662; NOTE: is split by file
# need to add PCM_ID
uniqueN(pcms_t[qValue <= 0.01, PeptideGroupID]) # 78638

uniqueN(psms_t[qValue <= 0.01, PCM_J_ID]) # 83589
uniqueN(pcmGrouper_psms_t[qValue <= 0.01, PCM_J_ID]) # 83662
# psm based results align

uniqueN(modPeps_t[qValue <= 0.01, unlist(PCM_J_ID)]) # 82865; higher than PCMs; overestimates
uniqueN(pcms_t[qValue <= 0.01, unlist(PCM_J_ID)]) # 78638
# 136657 reported by mokapot

round((uniqueN(psms_t[qValue <= 0.01, PCM_J_ID])/
         uniqueN(pcms_t[qValue <= 0.01, unlist(PCM_J_ID)]) * 100), 2) # 106.3

round((uniqueN(modPeps_t[qValue <= 0.01, unlist(PCM_J_ID)])/
         uniqueN(pcms_t[qValue <= 0.01, unlist(PCM_J_ID)]) * 100), 2) # 105.38
...
fwrite(bardata, file.path(figurePath, "figure-E3F-fdr.csv"))
