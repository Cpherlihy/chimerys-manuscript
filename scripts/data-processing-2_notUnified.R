...
readProteinsFromFastas <- function(pathToFasta){
  ...
  setcolorder(proteinsFromFasta, c("Accession", "FastaTitleLines", "ORGANISM"))
  return(proteinsFromFasta)
}

readPdResult_psmGrouper_dda <- function(pathToPdResult_dda, proteinsFromFasta){
  ...
  # check if peptide group quan values are a sum of all pcm quan values
  # NOTE: not the case in DDA
  ...
  # Flag entrapment
  pdresult[,ENTRAPMENT:=sapply(strsplit(ParentProteinAccessions, '; '), function(x) as.numeric(identical(unique(x), 'mimic')))]
  
  # ORGANISM mapping
  proteinsFromFasta[grepl('Random_[0-9]', FastaTitleLines),Accession:=FastaTitleLines]
  proteinsFromFasta[!grepl('Random_[0-9]', Accession),Accession:=sapply(FastaTitleLines, function(x) str_split(x, "\\|")[[1]][2])]
  proteinsFromFasta <- rbindlist(list(proteinsFromFasta,
                                      data.table(Accession='mimic',FastaTitleLines='>sp|mimic|mimic_ENTRAPMENT', ORGANISM='ENTRAPMENT')))
  ...
  if(!all(pdresult[ENTRAPMENT == 1,ORGANISM] == 'ENTRAPMENT')){
    print("No entrapment found.")
  }
  pdresult[DECOY == 1,ORGANISM:='DECOY']
  # table(pdresult$ORGANISM)
...
readPdResult_localPcmGrouper <- function(pathToPdResult, pathToMs8, proteinsFromFasta){
  cat('Reading in data from pdresult\n')
  ...
  pdresult[grepl(';', ParentProteinAccessions),ORGANISM:=unlist(pbmclapply(strsplit(ParentProteinAccessions, '; '), function(i) paste0(proteinsFromFasta[.(i),sort(unique(ORGANISM))], collapse = ';'), mc.cores = 10))]
  if(!all(pdresult[ENTRAPMENT == 1,ORGANISM] == 'ENTRAPMENT')){
    print("Organisms not assigned, correctly.")
  }
  pdresult[DECOY == 1,ORGANISM:='DECOY']
  # table(pdresult$ORGANISM)
  pdresult[,ParentProteinAccessions:=gsub(' ', '', ParentProteinAccessions)]
...
readDiann <- function(pathToTsv, proteinsFromFasta) {
  ...
  diann <- rbindlist(list(diann[,list(ID=Precursor.Id, PEPTIDE_SEQUENCE=Stripped.Sequence, PROTEIN_ACCESSION=Protein.Ids, Q_VALUE=Q.Value, SVMSCORE=CScore, DECOY=0, SAMPLE=File.Name, CHARGE=Precursor.Charge, MODIFIED_SEQUENCE=Modified.Sequence, QUAN=Precursor.Quantity, FRAGMENTS_USED=Fragment.Quant.Raw, SPECTRUM_SIMILARITY=Spectrum.Similarity, ORGANISM=Protein.Names, SOFTWARE='DIA-NN')],
                          diann[,list(ID=Precursor.Id, PEPTIDE_SEQUENCE=Stripped.Sequence, PROTEIN_ACCESSION=Protein.Ids, Q_VALUE=Q.Value, SVMSCORE=Decoy.CScore, DECOY=1, SAMPLE=File.Name, CHARGE=Precursor.Charge, MODIFIED_SEQUENCE=Modified.Sequence, QUAN=Precursor.Quantity, FRAGMENTS_USED=Fragment.Quant.Raw, SPECTRUM_SIMILARITY=Spectrum.Similarity, ORGANISM=Protein.Names, SOFTWARE='DIA-NN')]))
  ...
  diann[,ORGANISM:=unlist(pbmclapply(sapply(strsplit(gsub(pattern = '(?<=^|;)[A-Za-z0-9]*_', '', ORGANISM, perl = T), ';'), function(j) sort(unique(j))), function(i) paste0(i, collapse = ';'), mc.cores = 10))]
  
  # ORGANISM mapping
  # proteinsFromFasta <- copy(proteinsPeptideFasta)
  proteinsFromFasta[grepl('Random_[0-9]', FastaTitleLines),Accession:=FastaTitleLines]
  proteinsFromFasta[!grepl('Random_[0-9]', Accession),Accession:=sapply(FastaTitleLines, function(x) str_split(x, "\\|")[[1]][2])]
  setkey(proteinsFromFasta, Accession)
  
  setkey(diann, PROTEIN_ACCESSION)
  diann[,DIANN_ORGANISM:=ORGANISM]
  diann[,ORGANISM:=NULL]
  diann[proteinsFromFasta,ORGANISM:=ORGANISM]
  ...
  # table(diann$ORGANISM)
  diann[ORGANISM!='ENTRAPMENT',ENTRAPMENT:=0]
  diann[ORGANISM=='ENTRAPMENT',ENTRAPMENT:=1]
  ...
  setcolorder(diann, neworder = c('ID', 'PEPTIDE_SEQUENCE', 'PROTEIN_ACCESSION', 'Q_VALUE', 'SVMSCORE', 'DECOY', 'CHARGE', 'MODIFIED_SEQUENCE', 'QUAN', 'SAMPLE', 'FRAGMENTS_USED', 'SPECTRUM_SIMILARITY', 'DIANN_ORGANISM', 'ORGANISM', 'ENTRAPMENT', 'PCM_ID', 'PCM_J_ID', 'MODPEP_ID', 'MODPEP_J_ID', 'SOFTWARE'))
  diann <- unique(diann)
  
  return(diann)
}

readSpectronaut <- function(pathToExport, proteinsFromFasta){
  ...
  sn[,SN_ORGANISM:=NA]
  # sn[,ORGANISM:=NULL]
  sn[proteinsFromFasta,ORGANISM:=ORGANISM]
  ...
  setcolorder(sn, neworder = c('ID', 'PEPTIDE_SEQUENCE', 'PROTEIN_ACCESSION',
                               ...
                               'SN_ORGANISM', 'ORGANISM', 'ENTRAPMENT',
                               'PCM_ID', 'PCM_J_ID',
...
readSpectronaut_flagImputation <- function(pathToExport, proteinsFromFasta){
  cat('Reading in data from Spectronaut export\n')
  sn <- fread(pathToExport, stringsAsFactors = F, integer64 = 'double')
...
setEntrapmentFDR_local <- function(localPrecursors) {
  ...
  uniqueSVM <- localPrecursors[,list(TARGET=sum(DECOY==0 & ENTRAPMENT==0),
                                     DECOY=sum(DECOY==1 & ENTRAPMENT==0),
                                     ENTRAPMENT_TARGET=sum(DECOY==0 & ENTRAPMENT==1),
                                     ENTRAPMENT_DECOY=sum(DECOY==1 & ENTRAPMENT==1)),by=list(SOFTWARE, SAMPLE, SVMSCORE)]
...
readPdResult_localPcmGrouper_diffEntrMethods <- function(pathToPdResult, pathToMs8, proteinsFromFasta, digest = NULL){
  cat('Reading in data from pdresult\n')
...
readDiann_diffEntrMethods <- function(pathToTsv, proteinsFromFasta, digest = NULL) {
  cat('Reading in data from dia-nn tsv\n')
  diann <- fread(pathToTsv, stringsAsFactors = F, integer64 = 'double')
  files <- diann[,list(File.Name=unique(File.Name))]
  files[,newname:=gsub('\\..*', '', sapply(strsplit(File.Name, '\\\\'), 'tail', 1))]
  ...
  setkey(diann, File.Name)
  diann[files,File.Name:=newname]
  diann <- rbindlist(list(diann[,list(ID=Precursor.Id, PEPTIDE_SEQUENCE=Stripped.Sequence, PROTEIN_ACCESSION=Protein.Ids, LIB_Q_VALUE=Lib.Q.Value, Q_VALUE=Q.Value, SVMSCORE=CScore, DECOY=0, SAMPLE=File.Name, CHARGE=Precursor.Charge, MODIFIED_SEQUENCE=Modified.Sequence, QUAN=Precursor.Quantity, FRAGMENTS_USED=Fragment.Quant.Raw, SPECTRUM_SIMILARITY=Spectrum.Similarity, ORGANISM=Protein.Names, SOFTWARE='DIA-NN')],
                          diann[,list(ID=Precursor.Id, PEPTIDE_SEQUENCE=Stripped.Sequence, PROTEIN_ACCESSION=Protein.Ids, LIB_Q_VALUE=Lib.Q.Value, Q_VALUE=Q.Value, SVMSCORE=Decoy.CScore, DECOY=1, SAMPLE=File.Name, CHARGE=Precursor.Charge, MODIFIED_SEQUENCE=Modified.Sequence, QUAN=Precursor.Quantity, FRAGMENTS_USED=Fragment.Quant.Raw, SPECTRUM_SIMILARITY=Spectrum.Similarity, ORGANISM=Protein.Names, SOFTWARE='DIA-NN')]))
  ...
  if (!is.null(digest)) {
    ...
    diann[,DIANN_ORGANISM:=ORGANISM]
    diann[,ORGANISM:=NULL]
    diann[digest,ORGANISM:=i.ORGANISM]
  } else {
    # ORGANISM mapping
    # proteinsFromFasta <- copy(proteinsPeptideFasta)
    proteinsFromFasta[grepl('Random_[0-9]', FastaTitleLines),Accession:=FastaTitleLines]
...
readDigest <- function(pathToDigest) {
  ...
  digest_unique <- digest[!alldup(Sequence),]
  digest_nonunique <- digest[alldup(Sequence),]
  digest_nonunique_concat <- digest_nonunique[,list(ORGANISM=paste0(sort(unique(ORGANISM)), collapse = ';')),by=Sequence]
  digest_nonunique_concat[ORGANISM%like%';',ORGANISM:=gsub('ENTRAPMENT', '', ORGANISM)]
  digest_nonunique_concat[ORGANISM%like%';',ORGANISM:=gsub('^;|;$|(?<=;);', '', ORGANISM, perl = T)]
  digest <- rbindlist(list(digest_unique, digest_nonunique_concat))
  return(digest)
}

readSpectronaut_diffEntrMethods <- function(pathToExport, proteinsFromFasta, digest = NULL){
  cat('Reading in data from Spectronaut export\n')
  sn <- fread(pathToExport, stringsAsFactors = F, integer64 = 'double')
...
readSpectronaut_flagImputation_diffEntrMethods <- function(pathToExport, proteinsFromFasta, digest = NULL){
  cat('Reading in data from Spectronaut export\n')
  sn <- fread(pathToExport, stringsAsFactors = F, integer64 = 'double')
