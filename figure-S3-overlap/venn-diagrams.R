...
data_venn_psm_duell <-
  rbind(data_psm[condition %in% psm_se, .(condition = factor("7 other\nsearch\nengines"), .N), by = c("psm_J")],
        data_psm[condition %in% "CHIMERYS", .(condition = factor("CHIMERYS"), .N), by = c("psm_J")])
write_fst(data_venn_psm_duell, file.path(figurePath, "data_venn_psm_duell.fst"))
...
data_venn_ptm_native_duell <-
  rbind(data_psm[condition %in% ptm_native_se, .(condition = factor("4 other\nsearch\nengines"), .N), by = c("ptm_group_J")],
        data_ptm[condition %in% "CHIMERYS", .(condition = factor("CHIMERYS"), .N), by = c("ptm_group_J")])
write_fst(data_venn_ptm_native_duell, file.path(figurePath, "data_venn_ptm_native_duell.fst"))


#PTM group level - all ====
ptm_native_se <- c("MaxQuant", "Comet", "SequestHT", "MSAmanda")
data_ptm_joined <-
  rbind(data_ptm[condition %in% ptm_native_se, .(condition = factor("7 other\nsearch\nengines"), .N), by = c("ptm_group_J")],
        data_pcm_ptm[condition %in% "MSF-200", .(condition = factor("7 other\nsearch\nengines"), .N), by = c("ptm_group_J")],
        data_psm_ptm[condition %in% c("Metamorpheus", "MS-GF-plus"),
                     .(condition = factor("7 other\nsearch\nengines"), .N), by = c("ptm_group_J")])
data_venn_ptm_all_duell <- rbind(data_ptm_joined,
                                 data_ptm[condition %in% "CHIMERYS", .(condition = factor("CHIMERYS"), .N), by = c("ptm_group_J")])
write_fst(data_venn_ptm_all_duell, file.path(figurePath, "data_venn_ptm_all_duell.fst"))
