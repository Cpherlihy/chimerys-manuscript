...
This document describes how the data analysis and plots for figure 4 were generated. To recreate the figures, make sure to download all input files (available on [PRIDE](https://www.ebi.ac.uk/pride/archive?keyword=PXD053241)), place them under `dataPath` (adjust in `load-dependencies.R` to your own folder structure) and generate intermediate results in the linked `.R` scripts.
...
Intensities were exported directly from Skyline and matched to CHIMERYS intensity values.
...
Same density plot as above, but this time keeping Methionine. This is to show that Methionine-containing peptides impact all search engines equally negatively.

[R code to generate input file `figure-4D-density-keepMet.csv`](figure-4D-ma-keep-Met.R)
