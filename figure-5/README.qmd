...
This document describes how the data analysis and plots for figure 5 were generated. To recreate the figures, make sure to download all input files (available on [PRIDE](https://www.ebi.ac.uk/pride/archive?keyword=PXD053241)), place them under `dataPath` (adjust in `load-dependencies.R` to your own folder structure) and generate intermediate results in the linked `.R` scripts.
...
[R code to generate all input `.csv` files](figure-5AtoH-search-engine-results.R)
